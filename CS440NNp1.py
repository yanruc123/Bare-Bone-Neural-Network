...
class NeuralNetwork:
    ...
    def fit(self, X, Y, learningRate, epochs, regLambda):
        ...
        for e in epoch:
            ...
            for i in range(0,len(X)):
                ...
                forward_output.append(forward(sample))
                
                # 2. Backward progate once. Use the function "backpropagate" here!
                back_output.append(backpropagate(sample))
    ...
    def forward(self, X):
        ...
        self.hiddenLayer1 = sigmoid(np.dot(X,W1))
        self.hiddenLayer2 = np.dot(self.hiddenTrans1,W2)
        self.output = sigmoid(self.hiddenLayer2)
